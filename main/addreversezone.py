...
def create_network(infoblox_url, auth, network_data):
    """
    Create a new network in Infoblox with "Automatically Create Reverse-Mapping Zone" enabled and DHCP disabled.
    ...
    payload = {
        ...
        "disable": True,
        "auto_create_reversezone": True  # Automatically create Reverse-Mapping Zone
    }
    try:
...
def main():
    ...
    for network_data in config['networks']:
        ...
        if network_ref:
            ...
            if create_response:
                print(f"Network {network} created successfully with Reverse-Mapping Zone and DHCP disabled.")
